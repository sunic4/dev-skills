---
name: "cc-arch"
description: "系统架构设计与技术决策。技术选型/系统设计/接口定义/架构决策。输出到 arch/。"
---
...
### ✅ 必须满足（任一）
- [ ] 有对应的 REQ 文档 (`req/{slug}.md` 存在且 `status ≠ deprecated`)
- [ ] 用户明确的设计目标 + 足够的需求上下文

### ⚠️ wiki/ 不存在时
1. 自动创建 `arch/`, `arch/adrs/`, `arch/modules/`, `raw/research/`, `spikes/`
2. 继续正常执行
3. 提示用户建议后续执行 `cc-init`
...
title: "{标题}"  depends_on: ["req/{slug}.md"]
...
| 设计完成，已有项目 | → `cc-feat` |
| 需求不清 | ↩ 回 `cc-req` |
| 多需求共享架构 | depends_on 中列出所有 REQ |

### 处理 Roadmap 的 Shared Types

当 `shared-types.md` 含 `source: auto-generated-by-roadmap` 时:
